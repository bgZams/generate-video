window.initVideoLongGenerator = function() {
    const topicInput = document.getElementById('long-topic');
    const generateBtn = document.getElementById('generate-long-btn');
    ...
    if (generateBtn) {
        generateBtn.addEventListener('click', async () => {
            ...
            const generationOptions = {
                topic: topic,
                videoMode: 'long', // Force 'long' mode
                provider: aiConfig.provider,
                aiApiKey: aiConfig.apiKey,
                model: aiConfig.model,
                // Global settings
                voice: getGlobalSetting('narrator_voice', 'edge-id-gadis'),
                speed: getGlobalSetting('narrator_speed', 1.0), // Default speed for long video
                bgmMood: getGlobalSetting('bgm_mood', 'cinematic'), // Default mood for long video
                visualEffect: getGlobalSetting('visual_effect', 'cinematic'), // Default effect
                vignette: getGlobalSetting('vignette', false),
                privacyStatus: getGlobalSetting('youtube_privacy', 'unlisted'), // Default privacy for uploads
            ...
            console.log('Generating Long Video with options:', generationOptions);
            ...
            try {
                ...
                if (data.success) {
                    alert('Proses pembuatan video Panjang telah dimulai di background!');
